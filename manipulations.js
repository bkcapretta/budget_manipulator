...
var percentRange = form.getRange('D5:D14');
var fixedRange = form.getRange('G5:G14');
var done = form.getRange('J2');
...
function copy() 
{
  listOfPercent = percentRange.getValues();
  Logger.log("Percents: " + listOfPercent);
  Logger.log("About to check and convert");
  checkAndConvert();
}

// Purpose: to check if the total percents add to 100% or if the given dates are appropriate; if so, to convert into effort in money
function checkAndConvert() 
{
  ...
  if (salary == '') {
    ...
    ui.alert('You end date is before your start date. Please enter an appropriate timeline');
  }
  else {
    convertToMoney();
    done.setValue('Yes');
    ui.alert('You are ready to edit numbers in Percent Allocated % and Yearly Rate $ to see adjustments.');
  }
}
...
function onEdit(e) 
{ 
  // if user tries to edit something where everything else is fixed, return value to old value
  if (withinRange(e) && done.getValue() == 'Yes') {
    if (numberUnfixed() <= 1) {
      ui.alert('You cannot edit this cell since all other grants are fixed');
      e.range.setValue(e.oldValue);
    }
    else { // only do stuff to cells between D5 and E14 and if user has inputted info and pressed done
...
// Purpose: to count the number of fixed grants marked by user
...
function adjustValues(newVal, oldVal, cellRow, cellCol)
{
  ...
  if (form.getRange('D15').getValue() != 1.0 || form.getRange('E15').getValue() != yearly_salary) { 
    ...
    distributeChange(diff, cellRow, cellCol, sum);
...
function distributeChange(diff, cellRow, cellCol, sum) 
{
